...
## 7 Задание  
[ссылка на задание](https://github.com/MikhailNikitin99/TF_hw/tree/terraform-hotfix/src/remote_state)  

Немного не ясно как это использовать. Если у меня уже remote state на s3 бэкенде, то при поднятии он создает второй бакет и нужно через команды перекидывать на только что созданный бакет?  
Не понял как можно( и можно ли вообще ) менять бэкенд при поднятий инфраструктуры. Тоесть, сначала поднимается все с tfstate локальным(или другом удаленном), затем как все поднялось, создался сервисный акк и выдались ему права, я просто должен `terraform init -migrate-state` сделать для переноса на свежесозданный бакет? 
